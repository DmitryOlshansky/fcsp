...
struct TrackPath: public default_bfs_visitor {
	...
	TrackPath(ChemGraph& graph, ChemGraph::vertex_descriptor t) :g(graph), tgt(t){}
...
void linearDescriptors(ChemGraph& graph, std::unordered_map<int, LevelOne> &order1)
{
	...
	for (auto i = vrtx.first; i != vrtx.second; i++)
	{
		...
		if (j != order1.end())
		{
			cout << *i << ": " << atomSymbol(j->second.center) << " DC:" << j->second.index << endl;
	...
	{
		breadth_first_search(graph, dcs[j].first, buf, TrackPath(graph, dcs[i].first), get(&Atom::color, graph));
		auto vtx = vertices(graph);
		/*{
			for (auto i = vtx.first; i != vtx.second; i++)
				cout << *i << ": " << graph[*i].path << endl;
		}*/
		//TODO: mark if the path contains dual links (0 --> x)
		cout << dcs[i].second << " " << graph[dcs[i].first].path - 1 << " " << dcs[j].second << " " << 0 << endl;
	}
}

void dumpGraph(ChemGraph& graph, const char* file)
{
	ofstream ofs(file);
...
int main(int argc, const char* argv[])
{
	...
	//for(int i=1; i<argc; i++)
	{
		...
		if(!f){
			cerr << "WARNING: cannot open " << argv[i] << endl;
			continue;
		}*/
		try{
			/*vector<SDF> sdf = readSdf(cin);
			for(auto& e : sdf)
			{
				cout << "====="<<endl;
				for(auto& p : e.props)
				{
					cout << p.first << ":"<<p.second.front() << endl;
				}
				if(e.props["NAME"].front() == argv[1])
				{
					process(e.mol);
				}
			}*/
			ifstream descr1("descr1.csv");
			auto order1 = read1Order(descr1);
			CTab tab = readMol(cin);
			ChemGraph g = toGraph(tab);
			processCycles(g);
			linearDescriptors(g, order1);
			dumpGraph(g, "out.dot");
			//writeMol(tab, cout);
		}
